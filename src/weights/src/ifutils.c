		 ...
		 uint **doc_card, uint *corpsize)
{
     ...
     uint *dcard = NULL;// document cardinalities
     ...
     while ((read = getline(&line, &len, fp)) != -1) {
	  ...
	  f = (int **) realloc(f, (csize + 1) * sizeof(int *));
	  f[csize] = (int *) malloc((term - 1) * sizeof(int));
 ...
 */
void read_vocab(char **filename, uint **vocab, uint **termid, 
		uint **corpfreq, uint **docfreq, uint *vocsize)
{
     ...
     while ((read = getline(&line, &len, fp)) != -1){
	  // term
	  char *token = strtok (line," = ");
	  v = (uint *) realloc(v, (vsize + 1) * sizeof(int));
	  v[vsize] = atoi(token);
	  // term id
	  token = strtok (NULL, " = ");
	  id = (uint *) realloc(id, (vsize + 1) * sizeof(int));
	  id[vsize] = atoi(token);
	  // term frequency
	  token = strtok (NULL, " = ");
