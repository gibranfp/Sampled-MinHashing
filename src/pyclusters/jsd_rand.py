...
def rand_jsd((ixkdocs_,docs,dprobs,dlogprobs)):
    ...
    docs_=docs[ixkdocs]
    dprobs_=dprobs[ixkdocs]
    dlogprobs_=dlogprobs[ixkdocs]
...
"""
    ...
    for cluster in args[1:]:
        ...
        idocs_rand=dict([(s, 
            [[random.choice(range(docs.shape[0])) 
                        for y in range(s)] 
                        for x in range(1)]) 
                        for s,vals in sizes.iteritems()])

        jsd_rand={}
        for s,ks in idocs_rand.iteritems():
            jsd_rand[s]=np.average([ rand_jsd((ixkdocs,docs,dprobs,dlogprobs))
                for ixkdocs in ks])

        for x in jsd_rand.iteritems():
            print '>>>>',x[0],x[0],x[1]
